...
export function Menu({ desktop, icon, name, href, setShowMenu }: MenuProps): React.JSX.Element {
    ...
    if (desktop)
        return (
            <NavLink
                ...
                className={({ isActive }) =>
                    cn(
                        'flex flex-row gap-x-4 items-center justify-between px-2 py-1 active:scale-95 transition rounded capitalize',
                        {
    ...
    return (
        <NavLink
            ...
            className={({ isActive }) =>
                cn(
                    'w-full flex flex-row gap-x-4 items-center justify-between p-2 active:scale-95 transition rounded capitalize',
                    {
                        'bg-zinc-900 text-white dark:bg-white dark:text-black': isActive,

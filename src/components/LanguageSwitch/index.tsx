...
export function LanguageSwitch({ className, type = 'dropdown' }: ThemeSwitchProps): React.JSX.Element {
            ...
            {menu.map(({lng, icon, name, onClick}: MenuType, index: number) => (
                <button
                    ...
                    className={cn(
                        ...
                        i18n.language === lng && 'bg-zinc-900 dark:bg-white text-white dark:text-black active:scale-95 transition rounded',
                    )}
                    onClick={onClick}
                >
    ...
    return (
        <Menu as='div' className={cn('relative w-24', className)}>
            <Menu.Button
                className='z-20 w-full flex px-4 p-2 flex-1 text-sm items-center justify-center bg-zinc-900 dark:bg-white text-white dark:text-black active:scale-95 transition rounded outline-none focus:outline-none'
            >
                <Icon>language</Icon>
            </Menu.Button>
            <Menu.Items
                            ...
                            {({ active }) => (
                                <button
                                    className={cn(
                                        ...
                                        i18n.language === lng && 'bg-zinc-900 dark:bg-white text-white dark:text-black active:scale-95 transition rounded',
                                        active && 'bg-zinc-900 dark:bg-white text-white active:scale-95 transition rounded',
                                    )}
                                    onClick={onClick}
                                >
                                    {icon}
